...
function showshelterimg() {
    ...
    if (shelterimg.files.length> 4) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'You must only upload 4 images 😢',
        })}
    else if (shelterimg.files.length <4) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'You must upload 4 images 😢',
        })}
        ...
        for (var i = 0; i <= shelterimg.files.length - 1; i++) {
            ...
            } else if (imagefile >0 && imagefile<4096){
...
//reset image preview if user reset input
function resetimg(){
    var defaultimg = "img/noimage.png";
    document.getElementById('thumbnailimg').value = null;
    document.getElementById('shelterimgid1').value = null;
    thumbnailpreview.src=defaultimg;
    simageimg1.src=defaultimg;
    simageimg2.src=defaultimg;
    simageimg3.src=defaultimg;
    simageimg4.src=defaultimg;
}